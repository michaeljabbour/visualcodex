...
function setupConfigHandlers() {
  ...
  ipcMain.handle('set-api-provider', (event, provider, apiKey) => {
    ...
    if (apiKey.trim() === '') {
      // If empty key, remove the provider
      delete providers[provider];
    } else {
      // Otherwise update/add the provider
      providers[provider] = apiKey;
    }
    
    store.set('apiProviders', providers);
    console.log(`API providers updated, count: ${Object.keys(providers).length}`);
    return true;
  });
  ...
  ipcMain.handle('get-models', async () => {
    ...
    try {
      ...
      // Verify the current default model is in the list
      const defaultModel = store.get('defaultModel') as string;
      if (defaultModel && !modelList.includes(defaultModel)) {
        console.warn(`Default model "${defaultModel}" not in available models list. Adding it.`);
        modelList.unshift(defaultModel);
      }
      
      return modelList;
    } catch (error) {
      console.error('Error retrieving models:', error);
      // Return minimal fallback list of models in case of error
